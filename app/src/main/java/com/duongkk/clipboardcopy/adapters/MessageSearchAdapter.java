...
public class MessageSearchAdapter extends RecyclerView.Adapter<MessageSearchAdapter.ViewHolder> {
    ...
    public MessageSearchAdapter(Context context, List<Message> listMessages, CallBackFirebase callbackFirebase) {
    ...
    private void showMenuText(final Message msg, View view, final int pos) {
        ...
        new MaterialDialog.Builder(context).title(R.string.message)
                ...
                .items(R.array.item_long_click_search)
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (appListSearch == null) {
                    appListSearch = listMessages;
                }
                if (charSequence != null) {
                    if (appListSearch != null && appListSearch.size() > 0) {
                        for (final Message appInfo : appListSearch) {
                            if (appInfo.getContent().toLowerCase().contains(charSequence.toString())) {
                                results.add(appInfo);
                            }
                        }
                    }
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                listMessages = (ArrayList<Message>) filterResults.values;
                notifyDataSetChanged();
            }
        };
    }
